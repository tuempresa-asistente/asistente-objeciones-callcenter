...
class AsistenteObjeciones {
    ...
    async buscarRespuestas() {
        ...
        if (!texto) {
            this.mostrarNotificacion('Por favor escribe una objección', 'warning');
            return;
        }
    ...
    // FUNCIÓN REAL - CONECTAR CON GOOGLE APPS SCRIPT
    ...
    ocultarLoading() {
        ...
        this.buscarBtn.textContent = '🚀 Buscar Respuestas';
    }
        ...
        if (!respuestas || respuestas.length === 0) {
            this.respuestasLista.innerHTML = `
                <div class="respuesta-item">
                    <p>No se encontraron respuestas específicas para esta objección.</p>
                    <p><strong>Sugerencia:</strong> Intenta con objeciones más comunes como "Es muy caro", "Lo pensaré", etc.</p>
                </div>
            `;
        } else {
    ...
    mostrarEjemplos() {
        ...
        let index = 0;
        setInterval(() => {
            this.objeccionInput.placeholder = `Escribe la objección del cliente ej: '${ejemplos[index]}'...`;
